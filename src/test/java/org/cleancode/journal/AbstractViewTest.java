 ...
 * <p>
 * The tests use Chrome driver (see pom.xml for integration-tests profile) to run integration tests on a headless Chrome.
 * If a property {@code test.use.hub} is set to true, {@code AbstractViewTest} will assume that the TestBench test is
 * running in a CI environment. In order to keep the this class light, it makes certain assumptions about the
 * CI environment (such as available environment variables). It is not advisable to use this class as a base class for
 * you own TestBench tests.
 * <p>
...
public abstract class AbstractViewTest extends ParallelTest {
    ...
    protected void assertThemePresentOnElement(WebElement element, Class<? extends AbstractTheme> themeClass) {
        ...
        Boolean hasStyle = (Boolean) executeScript("" + "var styles = Array.from(arguments[0]._template.content" + ".querySelectorAll('style'))" + ".filter(style => style.textContent.indexOf('" + themeName + "') > -1);" + "return styles.length > 0;", element);

        Assert.assertTrue("Element '" + element.getTagName() + "' should have" + " had theme '" + themeClass.getSimpleName() + "'.", hasStyle);
    }
}
